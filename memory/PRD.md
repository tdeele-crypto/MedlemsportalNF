...
- ✅ **2026-02-24**: NEW — **Gratis Deltagere** felt i "Tilføj deltager" og "Rediger tilmelding" dialogerne (mellem Antal medlemmer og Antal ikke-medlemmer). Gratis tæller med i total antal deltagere men rabatteres først på medlemmer, derefter på ikke-medlemmer i pris-beregningen. Backend validerer `num_free <= num_members + num_non_members` (HTTP 400 med dansk fejl). CSV-eksport indeholder "Gratis"-kolonne. Event header og deltagerliste viser "N gratis" når > 0. 6/6 pytest + fuld Playwright flow bestået.
